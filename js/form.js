...
function has3d(){
  ...
  function load_user_form()
  {
    ...
    for (key in cook)
    {
      ...
      if (cook[key].substring(0,9)=='formed' || cook[key].substring(1,10)=='formed')
      {
...
function save_user_form(id, real_id)
{
  ...
  document.cookie="formed_"+id+"=true&"+data+"; expires="+date.toUTCString();
}
...
function increment_form(idd)
{
  ...
  jQuery.ajax({
    dataType: 'json',
    type: "POST",
    url: formedJS.ajaxurl,
    data: 'id='+id[0]+'&action=formed_increment2',
    ...
    if (jQuery('.fileupload-input').length)
    {
      jQuery('.fileupload-input').fileupload({
        ...
        done: function (e, data)
        {
          ...
          jQuery('<i data-key="'+data.result.files.new_name+'" class="formed-trash" title="Delete"></i>').appendTo(data.fname).click(function()
          {
            jQuery.ajax({
              url: formedJS.ajaxurl,
              type: "POST",
              data: 'action=formed_delete_file&key='+encodeURIComponent(jQuery(this).attr('data-key')),
              success: function (response) {
                if (response=='Deleted')
                {
                  jQuery(data.fname).text('');
                  jQuery('<span style="color: red; font-weight: bold; margin-left: -20px"><i class="formed-ok" style="color: green"></i></span>').appendTo(data.fname);
...
jQuery('.nform .fileupload-input').attr('data-url', formedJS.server);
...
function submit_formed(id)
{
  ...
  jQuery.ajax({
    ...
    type: "POST",
    url: formedJS.ajaxurl,
    data: 'id='+fid+'&action=formed_submit&'+jQuery('#'+id).mySerialize(id)+jQuery('#'+id+' :input').not('.no_show').serialize()+'&title='+title+'&emails='+emails,
    success: function (response)
    {
...
/* Submit the Form */
function submit_formed_new(form_id, form_uniq, id)
{
  ...
  jQuery.ajax({
    ...
    type: "POST",
    url: formedJS.ajaxurl,
    data: 'id='+fid+'&action=formed_submit&'+jQuery('#'+id).mySerialize(form_id)+jQuery('#'+form_id+' :input').not('.no_show').serialize()+'&title='+title+'&emails='+emails+'&multi='+multi,
    success: function (response)
    {
      ...
      jQuery('#'+form_id+' .valid_show').css('display','none').parent().removeClass('has-error');
        ...
        {          for (var key in response)
          {
            if (response.hasOwnProperty(key)) 
            {
                ...
                jQuery('#'+form_id+' .valid_show.'+key).slideDown().parent().addClass('has-error');
            }   
          }
...
function update_date()
{
  ...
  jQuery('.datepickermdy').each(function(){
    ...
    {
      jQuery.get(formedJS.locale+"bootstrap-datepicker."+lang+".js", function(){
       jQuery('#'+id2).datepicker(
       {
        'format': dmy,
        'language': lang,
...
jQuery(document).ready(function () {
  ...
  jQuery('body').on('submit','.nform',function(event)
  {
    ...
    submit_formed_new(jQuery(this).attr('id'), id[0],id[1]);
    return false;
  });
  ...
  jQuery('.nform').on('click','.label_div',function(){
    ...
    // Pre-Load Image
    var img=new Image();
    img.src=formedJS.other+'/images/loader_4.gif';
