        ...
        add: function (e, data) 
        {
          ...
          jQuery('<i data-url="'+data.result.files[0].url+'" class="icon-trash" title="Delete"></i>').appendTo(data.fname).click(function(){
            ...
            jQuery.ajax({
              url: formedJS.ajaxurl,
              type: "POST",
              data: 'action=formed_delete_file&url='+encodeURIComponent(jQuery(this).attr('data-url')),
              success: function (response) {
                if (response=='Deleted')
                {
...
jQuery('.nform .fileupload').attr('data-url',formedJS.server);
...
function submit_formed(id)
{
  ...
  jQuery.ajax({
    ...
    type: "POST",
    url: formedJS.ajaxurl,
    data: 'id='+fid+'&action=formed_submit&'+jQuery('#'+id).mySerialize(id)+jQuery('#'+id+' :input').not('.no_show').serialize()+'&title='+title,
    success: function (response) {
